...
def test_compare_moegradients():
    ...
    grads_ref_finer = calc_moe_gradients_i6p(
        mole, one_dm, mf, occs, energies, maxstep=0.000001
    )
    for j in range(0, 2 * np.count_nonzero(occs)):
        for i in range(0, mole.natm):
            assert np.allclose(
                grads_ref[i][j, :], grads_ref_finer[i][j, :], rtol=1e-4, atol=1e-6
            )
    ...
    for j in range(0, 2 * np.count_nonzero(occs)):
        for i in range(0, mole.natm):
            assert np.allclose(
                grads_ref[i][j, :], grads2[i][j, :], rtol=1e-4, atol=1e-6
            )
