...
def calc_moe_gradients_4p(mole,one_dm: np.ndarray, mf, occs, energies, maxstep=0.0001, verbose=0):
    ...
    for i in range(0,mole.natm) :
        ...
        for j in range(0,3):
            ...
            grad_moe[i][:,j] = ( energies_mm + 8*energies_p - energies_pp - 8*energies_m)/ (12*h)
    return grad_moe
...
def internal_coordinates(mole):
    ...
    if len(zmat_atoms) > 2 :
        ...
        nics = 0
        raise moegerror("There are no possible derivatives with respect to nuclear coordinates, this system is an atom.")
            ...
            if abs(nb0) < 1e-7: # o0, o1, c in line
                ...
                if abs(nb1) < 1e-7:  # o0 o1 o2 in line
                    ...
                    if np.dot(np.cross(b1, b0), r1) < 0:
                        ...
                        a2 =-np.arccos(np.dot(b1, b0) / (nb0*nb1))
                    zmat.append([symb[j], p0, nr0, p1, a1*180/np.pi, p2, a2*180/np.pi])
                    j += 1
