...
def project(basis_2,basis_1):
    ...
    assert(np.allclose(coeff_2_proj @ s_1 @ np.linalg.inv(coeff_2_proj), s_2 ,rtol=1e-8,atol=1e-6))
...
def topyscf(mol,verb_lvl=0):
    ...
    if mol.nelec :    
        ...
        print("No number of electrons was explicitely recorded in the input file. WARNING!")
    if mol.charge :
        mole.charge = mol.charge
    else :
        print("No charge was explicitely recorded in the input file. WARNING!")
    coords = []
    coords = np.asarray([[mol.atnums[i],mol.atcoords[i]*0.52917721092] for i in range(mol.atnums.size)])
    mole.atom = coords
    if mol.obasis_name : 
    ...
    try:
        ...
        raise wrappyscf("The basis set name could not be understood by PySCF. Use a different basis set or set it manually.")
...
def get_one_dm(mol,mole,verb_lvl=0):
    ...
    try :
        if mol.mo[0] == 'unrestricted' :
           ...
           if verb_lvl > 0 : print('Total number of alpha electrons is {0}'.format(checkrdm(one_dm_a_2,s_2)))
           one_dm_b_2 = proj @ one_dm_b_1 @ np.linalg.inv(proj)
           if verb_lvl > 0 : print('Total number of beta  electrons is {0}'.format(checkrdm(one_dm_b_2,s_2)))
           one_dm_2 = one_dm_a_2 + one_dm_b_2
           if verb_lvl > 0 : print('Total number of electrons is {0}'.format(checkrdm(one_dm_2,s_2)))
           if np.allclose(one_dm_a_2,one_dm_b_2) :
               if verb_lvl > 1 : print("This will be treated a restricted wave function, alpha and beta orbitals are the same.")
               ...
               if verb_lvl > 2 : print("This is a true unrestricted wave function.")
               ...
               return np.array((one_dm_a_2,one_dm_b_2)), np.array((coeffs_a,coeffs_b)), np.array((occs_a,occs_b)), np.array((energies_a,energies_b))
        if mol.mo[0] == 'restricted' :   
           ...
           if verb_lvl > 2 : print("This is a restricted wave function.")
           if verb_lvl > 0 : print("Total number of electrons is {0}".format(checkrdm(one_dm_2,s_2)))
           return one_dm_2, coeffs, occs, energies
    except :
       raise wrappyscferror("Could not understand if the passed data is restricted or unrestricted. You may set it manually.")


def tellmem(a,name):
    print("Lets check the size of the array {0}: {1} elements, {2} size.".format(name,a.size,a.shape))
    pp.pprint(a)
...
def checkrdm(a: np.ndarray,b: np.ndarray):
    ...
    try :
        ...
        raise wrappyscferror("This density matrix does not seem to contain an integer number of electrons.")
    return round(tr,6)
...
def solver(mole,one_dm,mf,verb_lvl):
    ...
    try :
        ...
        if verb_lvl > 4 : print("RKS solver built.")
    except :
        if verb_lvl > 4 : print("RHF solver built.")
    nmf.verbose = verb_lvl
    nmf.max_cycle = 20
    nmf.conv_check = True
    nmf.run(one_dm)
    if verb_lvl > 3 : print("SCF converged: {0} , with total energy:{1} a.u.".format(nmf.converged, nmf.e_tot))
...
def calc_gradients_elec(mole,one_dm: np.ndarray, mf, coeffs,occs,energies):
 ...
 """
    ...
    if len(one_dm) == 2 :
        try :
            grad_elec = grad.UKS(mf).grad_elec(mo_energy=mo_energy,mo_coeff=mo_coeff,mo_occ=mo_occ)
        except :
            grad_elec = grad.UHF(mf).grad_elec(mo_energy=mo_energy,mo_coeff=mo_coeff,mo_occ=mo_occ)
    else :
        try :
            grad_elec = grad.RKS(mf).grad_elec(mo_energy=mo_energy,mo_coeff=mo_coeff,mo_occ=mo_occ)
        except :
            grad_elec = grad.RHF(mf).grad_elec(mo_energy=mo_energy,mo_coeff=mo_coeff,mo_occ=mo_occ)
    return grad_elec
